...
namespace ConsoleGUI.Inputs
{
    public class TextBox : Input
    {
        ...
        public TextBox(Window parentWindow, int x, int y, string iD, int length = 38) : base(parentWindow, x, y, length, 1, iD)
        {
            Selectable = true;
        }

        public TextBox(Window parentWindow, int x, int y, string text, string iD, int length = 38) : base(parentWindow, x, y, length, 1, iD)
        {
            Text = text;
        ...
        public override void Draw()
        {
            ...
            WindowManager.WriteText(ref WindowBuffer, clippedPath + " ", new() { textColor = TextColour, backgroundColor = BackgroundColour });
            if (Selected)
                ShowCursor();
        }
